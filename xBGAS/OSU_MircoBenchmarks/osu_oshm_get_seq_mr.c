#define BENCHMARK "OSU OpenSHMEM Get Message (Single-word) Rate (MB/s) Test"
                    ...
                    int iterations)
{
    ...
    if (v.me < v.pairs) {
        ...
        for (i = 0, offset = 0; i < iterations; i++, offset += size) {
            if ( size == 1) {
                buffer[offset] = xbrtime_uint8_g((uint8_t *)&buffer[offset], v.nxtpe);
            } else if (size == 2) {
                buffer[offset] = xbrtime_uint16_g((uint16_t *)&buffer[offset], v.nxtpe);
            } else if (size == 4) {
                buffer[offset] = xbrtime_uint32_g((uint32_t *)&buffer[offset], v.nxtpe);
            } else if (size == 8) {
                buffer[offset] = xbrtime_uint64_g((uint64_t *)&buffer[offset], v.nxtpe);
            }
            else if (size > 8) {
                for (int j = 0; j < size; j += 8) {
                    buffer[offset + j] = xbrtime_uint64_g((uint64_t *)&buffer[offset + j], v.nxtpe);
                }
            }
        }
